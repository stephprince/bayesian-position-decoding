...
savedfiguresdir = '\\neuro-cloud\labs\singer\Steph\Figures\decoding_5050_peakrate1_maxavgrate10_181127\';
...
for i = 1:size(dayindex) 
    ...
    if ~exist(filename)
        ...
        load(filename)
        placefieldstemp{dayindex(i,1)}{dayindex(i,2)} = placefields{dayindex(i,1)}{dayindex(i,2)};
    end
end
placefields = placefieldstemp;
...
for i = 1:size(dayindex,1)
    ...
    for recIdx = 1:numel(recs)
        ...
        [posActual positionEstimate eventTimes] = calcPositionEstimate190131(trainingdata_exclcriteria,testingdata,positionInfo,dayindex(i,:),recIdx,decodebinsize);        
...
%probability/chance
posEstAvg_alloverchance = posEstAvg_all*(360/PFbinsize);
%plot the average for that day
figure; imagesc('XData',[0:PFbinsize:360-PFbinsize],'YData',[0:PFbinsize:360-PFbinsize],'CData',posEstAvg_alloverchance)
xlim([0 360]); ylim([0 360])
xlabel('Actual position'); ylabel('Estimated position')
colorbar
filename = [savedfiguresdir 'ALLdecodedposvsestpos_avg_chance'];
saveas(gcf,filename,'png'); saveas(gcf,filename,'fig');    

%% get decoding errors for individual animals
for i = 1:size(dayindex,1)
    for recIdx = 1:numel(posEstAvg{dayindex(i,1)}{dayindex(i,2)})
        if ~isnan(posEstAvg{dayindex(i,1)}{dayindex(i,2)}{recIdx})
            posEstAvgtemp = cat(3, posEstAvgtemp, posEstAvg{dayindex(i,1)}{dayindex(i,2)}{recIdx});
...
%% concatenate only days with minimum number of place fields
placecellmin = 15;
posEstAvgtemp = [];
for i = 1:size(dayindex,1)
    if ~isempty(trainingdata_exclcriteria{dayindex(i,1)}{dayindex(i,2)}) && sum(~isnan(trainingdata_exclcriteria{dayindex(i,1)}{dayindex(i,2)}.rate(:,1))) > placecellmin %checks if there are at least X place fields
        for recIdx = 1:numel(posEstAvg{dayindex(i,1)}{dayindex(i,2)})
            if ~isnan(posEstAvg{dayindex(i,1)}{dayindex(i,2)}{recIdx})
                posEstAvgtemp = cat(3, posEstAvgtemp, posEstAvg{dayindex(i,1)}{dayindex(i,2)}{recIdx});
            end
...
xlabel('Actual position'); ylabel('Estimated position')
colorbar
filename = [savedfiguresdir 'ALLdecodedposvsestpos_avg_placecellmin' num2str(placecellmin)];