function [statsOutput decodingOutput] = calcReplayStatsCues(decodingOutput)
% SP 2.5.19 this function calcs replay stats using the
% decodingSequencesDurRipples output

%% remap the decoding output so each location is a distinct visual cue
newspatialprob = zeros(size(decodingOutput.spatialprob,1)/2,size(decodingOutput.spatialprob,2));
newspatialprob(1:45,:) = decodingOutput.spatialprob(1:45,:) + decodingOutput.spatialprob(46:90,:); %because degrees 1-45 and 46-90 repeat patterns
newspatialprob(46:90,:) = decodingOutput.spatialprob(91:135,:) + decodingOutput.spatialprob(136:180,:);
decodingOutput.spatialprob = newspatialprob;
