function [posActual positionEstimate timebins] = calcPositionEstimate190131(trainingdata,testingdata,positionInfo,dayindex,recIdx,binsize)
% SP 9.27.18
...
for eventIdx = 1:length(testingdata{dayindex(1)}{dayindex(2)}{recIdx}.blockdata)
    ...
    startpos = find(positionInfo(recIdx).timeSmooth == timebins{eventIdx}(1));
    endpos =  find(positionInfo(recIdx).timeSmooth == timebins{eventIdx}(end));
