function out = calcspikeratenormocc(processeddatadir, dayindex, behaviorindex, sessindex, binsize)
% SP 9.12.18
...
virmendatadir = [processeddatadir 'F' num2str(dayindex(1)) '_' num2str(dayindex(2)) '\' num2str(dayindex(4)) '\'];
...
for clusterIdx = 1:length(clusterInfo.labels)
    ...
    spikePositions(clusterIdx).position = getSpikePositions_stableFR(clusterIdx, recInfo, positionInfo, FRInfo, speedThreshold);
...
for clusterIdx = 1:length(clusterInfo.labels)
    stableTimes{clusterIdx} = FRInfo.stabletimes{clusterIdx};
    totalTimeInBin(clusterIdx,:) = getTimeSpentInPosition_stableFR(positionInfo, stableTimes{clusterIdx}, edges, recs);
...
for clusterIdx = 1:length(clusterInfo.labels)
    ...
    out.pos = edges;
    out.binsize = binsize;
    out.stabletimesall{clusterIdx} = FRInfotemp.stabletimes{clusterIdx}; %this gets all stable times not just ones during behavior sessions
end
